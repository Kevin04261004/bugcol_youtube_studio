...
globalThis.fetch=async(url,opts)=>{calls.push(String(url));return reply(String(url),opts);};
...
// 살아 있는 저장소 사본을 먼저 쓴다
reply=url=>url===BASE+'/index.html'?new Response('<!doctype html><p>새 화면'):new Response('Not found',{status:404});
let r=await get('/');
assert.equal(r.headers.get('X-Studio-Source'),'live');
assert.equal(r.headers.get('Content-Type'),'text/html; charset=utf-8');
assert.match(await text(r),/새 화면/);
assert.deepEqual(calls,[BASE+'/index.html']);
...
reply=url=>url===BASE+'/vendor/extra.js'?new Response('export const ok=1;'):new Response('Not found',{status:404});
...
reply=url=>url===BASE+'/build-id.txt'?new Response('abc123456789\n'):new Response('Not found',{status:404});
...
reply=url=>url===BASE+'/build-id.txt'?new Response(buildId):new Response('Not found',{status:404});
