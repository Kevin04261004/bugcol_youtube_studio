...
모바일과 PC에서 대사·녹음·이미지·영상을 직접 배치하는 유튜브 롱폼 편집기입니다.

## 직접 편집

- 대사 붙여넣기/TXT: 줄바꿈마다 장면 하나를 생성하며 자막 표시는 선택합니다.
- 이미지·영상 소재함, 여러 레이어, 드래그 이동과 모서리 크기 조절, 순서·잠금·숨김·복제.
- 타임라인 등장/종료 시간, 위/좌/우 진입·팝·페이드, 위치·크기·회전·불투명도 키프레임.
- 장면 순서 변경, 실행 취소/다시 실행, 녹음이 없는 장면의 길이 설정.
- WebCodecs H.264/AAC MP4 출력. 미지원 기기는 MediaRecorder 호환 모드(MP4 또는 WebM, 실시간 렌더링).
- 기존 로컬 자동 저장·서버 폴더·프로젝트 ZIP과 호환. 레이어/키프레임은 scene 안에 저장됩니다.

영상 소재의 원음은 사용하지 않으며 별도 녹음/가져온 음성을 사용합니다. 화면 흔들림 효과는 없습니다. 호환 모드는 전경 탭에서 영상 길이만큼 시간이 걸리며 기기 성능에 따라 프레임 속도가 달라질 수 있습니다.
...
- 문장 삭제: 녹음과 장면을 함께 정리하고, 이어가던 다음 문장은 그 장면을 물려받음. 문장 번호는 바뀌지 않음
- 음성 여러 개 또는 ZIP 가져오기: 숫자 파일명을 문장 번호에 매핑
- 문장별 숫자 WAV 파일과 편집 안내를 ZIP으로 내보내기
- 장면 파일 가져오기, 검수 후 WebCodecs H.264/AAC MP4 렌더링
- 앞 문장 장면 이어가기: 문장마다 화면이 바뀌지 않고 한 장면을 여러 문장에 걸쳐 유지. 영상은 끊기지 않고 이어서 재생되고 자막만 바뀜
- 영상 자르기 도구: 긴 원본(예: 1시간)을 열어 필요한 구간만 잘라 조각으로 저장하고 장면에 넣기. 원본은 프로젝트에 저장하지 않음
- 영상 소재에서 쓸 구간 고르기: 장면 설정에서 시작·끝 지점을 정하면 그 부분만 장면에 쓰임. 구간이 녹음보다 짧으면 마지막 화면이 멈춤
- 문장 타임라인에서 장면을 고르면 그 위치로 미리보기 이동, 전체 미리보기 중에는 그 지점부터 이어 재생
- 타임라인 장면 클릭만으로 검수 완료 (해제는 장면 설정의 검수 체크)
